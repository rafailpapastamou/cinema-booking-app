...
class MainActivity : AppCompatActivity() {
    ...
    val LANGUAGE_PREFERENCE_KEY = "language_preference"
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        title = getString(R.string.main_page_title);
        ...
        if (user==null){
            ...
            tvUserEmail.text = user.email // Display the user's email in the navigation drawer header
        }
        ...
        navView.setNavigationItemSelectedListener { menuItem ->
            when (menuItem.itemId) {
                ...
                R.id.nav_home -> {
                    // Close the navigation drawer (we are already in the main activity)
                    drawerLayout.closeDrawer(GravityCompat.START)
                }
                R.id.nav_admin -> {
                            ...
                            user?.reauthenticate(credential)
                                ?.addOnCompleteListener { reauthTask ->
                                    if (reauthTask.isSuccessful) {
                                        val userRef = usersRef.child(userId) // To delete the user from the realtime database as well
                                        userRef.removeValue()
                                        user.delete()
        ...
        btnMovies.setOnClickListener{
            val intent = Intent(applicationContext, Movies::class.java)
            startActivity(intent)
        }
    ...
    private fun showInfoDialog() { // Function for the pop up dialog info box
        val dialog = Dialog(this)
